...
{
    return update_status_and_return_true_if_ok(fmi2DoStep_(c, currentCommunicationPoint, communicationStepSize, noSetFMUStatePriorToCurrentPoint));
}




bool cd_library::cancel_step(fmi2Component c)
{
...
{
    return fmi2SetRealInputDerivates_(c, vr.data(), vr.size(), order.data(), value.data());
}
        ...
        )
{
    return update_status_and_return_true_if_ok(
            fmi2GetRealOutputDerivates_(c, vr.data(), vr.size(), order.data(), value.data());
}
