...
#include "fmup/PySlaveInstance.hpp"

#include "fmup/PyState.hpp"
...
std::unique_ptr<fmup::PyState> pyState = nullptr;
    ...
    const cppfmu::Logger& logger)
{
    ...
    if (pyState == nullptr) {
        pyState = std::make_unique<fmup::PyState>();
    }

    return cppfmu::AllocateUnique<fmup::PySlaveInstance>(
