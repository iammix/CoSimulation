...
from ..fmi2slave import Fmi2Slave, Boolean, Integer, Real, String
from fmup import __version__ as VERSION
...
def test_Fmi2Slave_generation_tool():
    ...
    xml = slave.to_xml()
    assert xml.attrib['generationTool'] == "fmup {}".format(VERSION)
...
def test_Fmi2Slave_getters(fmi_type, value):
    ...
    if type(value) is py_type:
        assert getattr(slave, "get_{}".format(fmi_type_name))([0]) == [value]
    else:
        with pytest.raises(TypeError):
            getattr(slave, "get_{}".format(fmi_type_name))([0])
...
def test_Fmi2Slave_setters(fmi_type, value):
    ...
    if type(value) is py_type:
        set_method = getattr(slave, "set_{}".format(fmi_type_name))
        set_method([0], [value])
        assert getattr(slave, "get_{}".format(fmi_type_name))([0]) == [value]
    else:
        set_method - getattr(slave, "set_{}".format(fmi_type_name))
...
def test_Fmi2Slave_log_categories():
    ...
    for category, description in Slave.log_categories.items():
        assert categories.find(" Category[@name='{}'][@description='{description}']") is not None
...
def test_Fmi2Slave_customized_log_categories(new_categories):
    ...
    if len(new_categories) > 0:
        assert len(categories) == len(Slave.log_categories)
        for category, description in Slave.log_categories.items():
            assert categories.find("Categories[@name='{}'][@description='{}']") is not None
        else:
