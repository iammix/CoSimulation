...
def get_model_description(filepath: Path, module_name: str) -> Tuple[str, Element]:
    ...
    if not isinstance(instance, Fmi2Slave):
        raise TypeError("The provided class {} does not inherit from {}".format(class_name, Fmi2Slave.__qualname__))
...
class FmuBuilder:
    ...
    def build_FMU(
        ...
        if not script_file.exists():
            raise ValueError("No such file {}".format(script_file))
        if not script_file.suffix.endswith(".py"):
            raise ValueError("File {} must have extension .py".format(script_file))
        ...
        if documentation_folder is not None:
            ...
            if not documentation_folder.exists():
                raise ValueError("The documentation folder does noe exists {}".format(documentation_folder))
        ...
        with tempfile.TemporaryDirectory(prefix="pythonfmu_") as tempd:
                ...
                if file_ == script_file.parent:
                    ...
                    for f in file_.iterdir():
                        if f.is_dir():
                            ...
                            logger.debug("Skip file with the same name as the script found in project file.")
                else:
                    if file_.is_dir():
            ...
            model_identifier, xml = get_model_description(temp_dir.absolute() / script_file.name, module_name)

            dest_file = dest / "{}.fmu".format(model_identifier)
            ...
            with zipfile.ZipFile(dest_file, "w") as zip_fmu:
                ...
                for f in itertools.chain(src.rglob(".hpp"), src.rglob("*.cpp"), src.rglob("CMakeLists.txt")):
                    relative_f = f.relative_to(src)
                    SubElement(source_node, "File", attrib={"name": relative_f.as_posix()})
                ...
                for f in itertools.chain(
                    ...
                    arcname = (
                            binaries
                            / relative_f.parent
                            / "{}{}".format(model_identifier, relative_f.suffix)
                    )
                    zip_fmu.write(f, arcname=arcname)

                # Add the documentation folder
                if documentantion_folder is not None:
                    documentation = Path("documentantion")
                ...
                zip_fmu.writestr("modelDescription.xml", xml_str.toprettyxml(encoding="UTF-8"))
            return dest_file

    @staticmethod
    def has_binary() -> bool:
        binary_folder = get_platform()
        lib_ext = get_lib_extension() or "*"
        src_binaries = HERE / "resources" / "binaries" / binary_folder
        return src_binaries.exists() and len(list(src_binaries.glob("*.{}".format(lib_ext)))) >= 1
...
def create_command_parser(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--doc",
        dest="documentantion_folder",
        help="Documentantion folder to include in the FMU.",
        default=None
    )
    for option in FMI2_MODEL_OPTIONS:
        ...
        parser.add_argument(
            "--{}".format(option.cli),
            dest=option.name,
            help="If given, {}={}".format(option.name, action[6:]),
            action=action
        )
    parser.add_argument(
        "project_files",
