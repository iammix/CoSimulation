...
import fmup
from fmup.builder import FmuBuilder, get_platform
...
def test_zip_content(tmp_path):
    ...
    with zipfile.ZipFile(fmu) as files:
        ...
        if FmuBuilder.has_binary():
            assert ("/".join(("binaries", get_platform(), f"PythonSlave.{lib_extension}")) in names)
        else:
            nfiles -= 1

        # Check sources
        src_folder = Path(fmup.__path__[0]) / "fmup-export"
        ...
        # Check fmup is embedded
        pkg_folder = Path(fmup.__path__[0])
        for f in pkg_folder.rglob("*.py"):
            relative_f = f.relative_to(pkg_folder).as_posix()
            if "test" not in relative_f:
                assert "/".join(("resources", "fmup", relative_f)) in names
