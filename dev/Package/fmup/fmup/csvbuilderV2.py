...
from fmup.fmi2slave import Fmi2Type, Fmi2Slave, Fmi2Causality, Fmi2Variability, Integer, Real, Boolean, String
    ...
    for type in Fmi2Type:
        if type.name in s_lower:
            if type == Fmi2Type.enumeration:
                raise NotImplementedError("Unsupported Type: {}".format(Fmi2Type.enumeration.name))
            else:
                return type
    raise TypeError("Could not process type from input string:{}".format(s))
...
class Header:
    ...
    def __repr__(self):
        return "Header(name={}, type={}".format(self.name, self.type.name)
...
class {classname}(Fmi2Slave):
    def __init__(self, **kwargs):
        ...
        def read_csv():
            with open(self.resources + '/' + "{filename}") as f:
                return list(csv.reader(f, skipinitilspace=True, delimiter=',', quotechar='"'))
    ...
    def find_indices(self, t, dt):
        current_t = self.timesp[self.current_index]
        while current_t < t:
...
class CsvFmuBuilder:
    ...
    def build_FMU(
        ...
        if not csv_file.suffix.endswith(".csv"):
            raise ValueError("File {} must have extension '.csv'".format(csv_file))

        options["dest"] = dest
        options["project_files"] = {csv_file}

        with tempfile.TemporaryDirectory(prefix="fmup_") as tempd:
            temp_dir = Path(tempd)
            script_file = tempd_dir / (csv_file.stem + ".py")
            with open(script_file, "+w") as f:
                f.write(create_csv_slave(csv_file))
            options["script_file"] = script_file
            return CsvFmuBuilder.build_FMU(**options)
