...
class Vehicle(object):
    ...
    def createStifnessMatrix(self, h):
        self.krr = self.mrr * (3.14159/(100*h))*(3.14159/(100*h))
        self.K[0,0] = self.k
        self.K[0,1] = - self.k
        self.K[1,1] = self.k
        self.K[1,0] = - self.k
    ...
    def setInterfaceInput(self, interface):
        self.constraint = interface
    ...
    def do_Step(self):
        ...
        for i in range(len(self.t) - 1):
            ...
            temp_2 = - np.matmul((bn * self.C + h * self.K), self.xdot[:,i])
            temp_3 = - np.matmul(((1 - bn) * self.C + h * h / 2 * (1 - 2 * an) * self.K), self.xddot[:,i])
            ...
            self.x[:,i+1] = self.x[:,i] + h * self.xdot[:,i] + h * h / 2 * ((1 - 2 * bn) * self.xddot[:,i] + (2 * bn) * self.xddot[:,i+1])
        
        return self.x, self.xdot, self.t

    def irregularity( self, V, seed, type):
        if type == 1:
            pass
...
#-------------Data------------------------
    ...
    v = np.array([irr_0 * omega, 0.0, 0.0])
