...
class Debugbridgefmuv2(Fmi2Slave):
    author = "iammix"
    description = "Bridge OpenSees model. See bridge.py file for more information about the model."

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.tag = 1
        self.number_of_div = 10
        self.velocity = 0.0

        self.register_variable(Real("tag", causality=Fmi2Causality.input))  # tag=0 for initial, tag=1 for restore
        # self.register_variable(Real("path", causality=Fmi2Causality.input))  # DataBase saving Path
        self.register_variable(Real("number_of_div", causality=Fmi2Causality.output))  # Bridge length division
        self.register_variable(Real("velocity", causality=Fmi2Causality.input))  # CS Communication time Interval
    ...
    def do_step():
        class Bridge(object):
            ...
            def set_interface_input(self, T, lc, nsteps):
                ...
                for i in range(0, self.nsteps + 1):
                    ...
                    ops.timeSeries('Path', i, '-values', 0, factor, 0, '-time', tstart, tstart + 2 * self.dt,
                                   4 * self.dt,
                                   "-prependZero")
                    ...
                    for j in range(1, self.nodenum + 1):
                        if ops.nodeCoord(j, 1) > xload:
                            next_id = j
                            break
                    ops.eleLoad('-ele', next_id - 1, '-type', '-beamPoint', 1,
                                (xload - ops.nodeCoord(next_id - 1, 1)) / (
        ...
        def main():
            ...
            k = np.array([0, k, 0])
            c = np.array([0, c, 0])
            IC = np.array([0, 0, 0, 0, 0, 0])  # [x1,x2,u1,u2,a1,a2]
            meshnum = 4
            velocity = 5
            length = 20
            section_area = 1
            ...
            youngmodulus = 1
            izz = 1
            # element number, velocity, Length, Section Area, MassPerLenght, Young Modulus, Izz, cs_timestep, tag
            B = Bridge(meshnum, velocity, length, section_area, massperlength, youngmodulus, izz)
            lc = np.array([-10, -10])
