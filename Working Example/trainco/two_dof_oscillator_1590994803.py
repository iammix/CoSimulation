...
def irregularity(tag, roughness, L, v):
    ...
    v_t = [i * v for i in t]  # corresponding place of the irregularity amplitude
    ...
    ax.legend()
    plt.show()
    return (v_t, qt) # return position of the irregularity amplitude and the amplitude itself
...
class Vehicle(object):
    ...
    def do_Step(self):
        ...
        for i in range(len(self.t) - 1):
            ...
            self.fcons[2] = self.mrr * (-self.constraint[0] * self.constraint[0] * \
                                        np.sin(self.constraint[0] * self.t[i + 1])) * self.constraint[1]
